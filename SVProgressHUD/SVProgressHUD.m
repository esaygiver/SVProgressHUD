...
@property (nonatomic) BOOL graceTimeEnabledForStatusUpdates;
@property (nonatomic, strong) NSTimer *fadeOutTimer;
@property (nonatomic, copy) NSUUID *pendingDismissalToken;
...
+ (void)setGraceTimeEnabledForStatusUpdates:(BOOL)graceTimeEnabledForStatusUpdates {
    [self sharedView].graceTimeEnabledForStatusUpdates = graceTimeEnabledForStatusUpdates;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        _graceTimeEnabledForStatusUpdates = YES;
...
- (void)showProgress:(float)progress status:(NSString*)status {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // Cancel pending dismissal
            strongSelf.pendingDismissalToken = nil;
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            if (self.graceTimeInterval > 0.0 && self.backgroundView.alpha == 0.0f && self.graceTimeEnabledForStatusUpdates) {
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(SVProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong SVProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
            NSUUID *dismissalToken = [[NSUUID alloc] init];
            strongSelf.pendingDismissalToken = dismissalToken;
            ...
            __block void (^completionBlock)(void) = ^{
                // Check that the dismissal wasn't cancelled while the fade out animation was occurring.
                // and the change of these values has not been cancelled in between e.g. due to a new show
                if([strongSelf.pendingDismissalToken isEqual:dismissalToken]) {
                    strongSelf.pendingDismissalToken = nil;
            ...
            dispatch_after(dipatchTime, dispatch_get_main_queue(), ^{
                ...
                if (strongSelf.fadeOutAnimationDuration > 0 && strongSelf.backgroundView.alpha > 0.0f) {
                    // Animate appearance
